...
for isv = trainval
  ...
  annTypes = {'sitstand' };
  if isValidation
    dataType='test'; annType=annTypes{1}; % specify dataType/annType
  else
    dataType='train'; annType=annTypes{1}; % specify dataType/annType
  end
...
%         if scale == -1 % connot compute scale
        ...
        joint_all(personCnt).isValidation = isValidation;
        joint_all(personCnt).sitstand = a.sitstand;
